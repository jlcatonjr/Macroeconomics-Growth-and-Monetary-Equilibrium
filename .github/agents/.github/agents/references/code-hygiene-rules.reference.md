...
**Enforcement check:**
```
find */outputs/ -name "*.backup" -o -name "*.bak" -o -name "*_old.*" \
...
### CH-03 — No Ad-Hoc Scripts in `*/outputs/`

Source directories contain only production code. Investigative, debug, fix, and benchmark scripts must not reside in `*/outputs/`.
